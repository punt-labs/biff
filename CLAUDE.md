...
[TESTING.md](TESTING.md) is the single source of truth for tier structure,
directories, markers, and the dev-box-vs-CI split — do not duplicate it here.
New features need tests at tiers 1-2 minimum; higher tiers for
integration-critical paths.

`uv run pytest` (default) runs tiers 1-2 only. Everything else needs an
explicit `-m <marker>` — see TESTING.md's "Running tests" section for the
full list and what each one requires locally.
...
**Take every comment seriously.** There is no such thing as "pre-existing" or "unrelated to this change" — if you can see it, you own it. If a reviewer (human or bot) flagged it, investigate and fix it. Re-push, re-watch CI, re-read reviews. Repeat until the **last review cycle is uneventful** — zero new comments, all checks green.
...
**Worker pool for biff** — Python with a NATS-backed relay. Each domain has a primary specialist plus alternates for overlapping areas; within each row, the worker and evaluator must be distinct handles.
