...
def create_server(state: ServerState) -> FastMCP[ServerState]:
    ...
    mcp: FastMCP[ServerState] = FastMCP(
        ...
        instructions=(
            ...
            "One command, multiple forms: `/biff:read 5m` starts polling every "
            "5m (sets the server cadence and schedules a recurring model check); "
            "`/biff:read n` stops; `/biff:read status` shows config; bare "
            "`/biff:read` checks now. Run `/biff:read 1m` once to stay "
