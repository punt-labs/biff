...
class TestCapture:
    def test_captures_pid_and_start_time(
        ...
        monkeypatch.setattr(sid_mod, "_process_start_time", lambda _pid: 999.0)
...
class TestResolveRoutingId:
    ...
    def test_resume_reuses_session_id(
        ...
        monkeypatch.setattr(sid_mod, "_process_start_time", lambda _pid: 111.0)
        SessionHint(
            session_id="2f5a1c3e-1b2d-4e5f-8a9b-0c1d2e3f4a5b",
            claude_pid=57369,
    ...
    def test_recycled_pid_rejected(
        ...
        monkeypatch.setattr(sid_mod, "_process_start_time", lambda _pid: 222.0)
        SessionHint(
    ...
    def test_malformed_session_id_rejected(
        ...
        monkeypatch.setattr(sid_mod, "_process_start_time", lambda _pid: 5.0)
        SessionHint(
    ...
    def test_absent_hint_under_claude_returns_none(
        ...
        _use_tmp_data_dir(monkeypatch, tmp_path)
        monkeypatch.setattr(sid_mod, "topmost_claude_pid", lambda: 42)
        monkeypatch.setattr(sid_mod, "_RESOLVE_ATTEMPTS", 2)
        monkeypatch.setattr(sid_mod.time, "sleep", lambda _s: None)
        assert SessionHint.resolve_routing_id() is None
