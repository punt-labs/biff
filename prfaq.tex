...
% Label FAQ pairs: \begin{faqpair}{Question}\label{faq:slug}
% Then cross-reference: (see \faqref{faq:slug})
\newcounter{faqnum}
...
\newcommand{\prfaqcurrentversion}{\prfaqversion{2}{7}}
...
engineers message teammates, check presence, see what their agents are doing,
and broadcast to their team --- all without leaving the terminal. Unlike Slack
and Discord, which demand a separate window and constant attention, biff runs
inside the engineer's existing Claude Code session as MCP-native slash
commands. Engineers type \texttt{/write @kai} the same way they type
...
all-day-online knowledge workers, not for makers in deep focus. Worse,
those tools show \emph{everything} --- every channel, every project,
every server --- regardless of which codebase is open in the terminal
right now.
...
catch up on, no emoji reactions to parse. Communication is repo-scoped and
pull-based: when you open a project, you see only the teammates and messages
for \emph{that} project --- not the ten others on the back burner. When
...
them?'') and your terminal's status bar shows a live indicator ---
\texttt{kai:tty1(3)} in bold yellow when you have unreads,
\texttt{kai:tty1(0)} when caught up, \texttt{kai:tty1(n)} when messages are
off. You decide when to engage. The full command reference is in Appendix~A.

Because biff speaks MCP, agents participate alongside humans as natural
members of the team. An autonomous coding agent can \texttt{/plan} what it is
working on, \texttt{/write} a human when it needs a decision, and show up in
\texttt{/who} alongside everyone else. Each agent gets a distinct identity via
\texttt{/tty} sessions, and \texttt{/who} shows host and directory per session
so you can see where each agent is working. Workflow hooks tie into your
development process: claiming a task auto-sets your plan, creating a PR
triggers a \texttt{/wall} announcement. All messages are end-to-end encrypted
via NaCl/libsodium, so the relay never sees message contents. Biff is
designed for the humans who work alongside agents, not for agents that work
without humans --- the same intentional, human-scale communication vocabulary
that made BSD utilities great, updated for teams where some of the members
happen to be AI.
...
\texttt{uv tool install punt-biff}, then \texttt{biff install} to register
the MCP server and slash commands, then \texttt{biff doctor} to verify the
setup. Restart Claude Code, and you are live. Biff ships with a shared relay
on Synadia Cloud with end-to-end encryption, so there is zero infrastructure
to provision. Team configuration lives in \texttt{.punt-labs/biff/config.yaml}
committed to your git repository --- it only needs a team member list; the
relay URL defaults to the shared relay. Your display name is resolved
automatically from your GitHub identity. When you clone a repo that has
that file, biff automatically connects you to the right team. There is no account
...
Install with \texttt{uv tool install punt-biff}, then run \texttt{biff install}
...
\begin{faqpair}{What is biff and who is it for?}\label{faq:what-is-biff}
  ...
  biff is for you. It provides twelve slash commands covering messaging,
  presence, broadcast, session identity, and availability control --- all without
  leaving your session. See Appendix~A for the full command reference.
\end{faqpair}

\begin{faqpair}{How is biff different from Slack?}\label{faq:diff-slack}
  ...
  And because team configuration lives in \texttt{.punt-labs/biff/config.yaml}
  committed to the repo, communication is scoped to the project you are working on right
  now. Slack shows you every channel from every project simultaneously; biff
  shows you only the teammates and messages relevant to the code in your
  terminal.
\end{faqpair}

\begin{faqpair}{How is biff different from Discord?}\label{faq:diff-discord}
...
\begin{faqpair}{How do I get started?}\label{faq:get-started}
  Run \texttt{uv tool install punt-biff}, then \texttt{biff install} to register
  the MCP server and slash commands, then \texttt{biff doctor} to verify the
  setup. Restart Claude Code. Biff ships with a shared demo relay on Synadia
  Cloud, so there is no infrastructure to provision. If your repo has
  \texttt{.punt-labs/biff/config.yaml} (committed by a teammate), biff picks up the team
...
\begin{faqpair}{What happens to my messages? Is communication private?}\label{faq:privacy}
  All direct messages are end-to-end encrypted using NaCl/libsodium. The
  relay stores only ciphertext it cannot read. Biff follows a POP mail
  model: messages are held only until the recipient reads them, then
  discarded. Biff is not a system of record. The shared relay is hosted on
  Synadia Cloud; teams that want data sovereignty can self-host the same
  NATS server and route all traffic through their own infrastructure. Biff
  never analyzes or trains on your messages.
\end{faqpair}

\begin{faqpair}{Does biff work without Claude Code?}\label{faq:without-claude}
...
\begin{faqpair}{Will biff support real-time pairing or live conversation?}\label{faq:realtime}
  \texttt{/talk} is shipped with a BSD-style two-phase handshake. The inviter
  waits (``Waiting for @user to respond\ldots''), the target accepts, and both
  enter a modal conversation. Messages are ephemeral (NATS core pub/sub, no
  inbox). Either side types \texttt{end} to hang up both sides---mutual
  hangup, just like the original BSD \texttt{talk(1)}.

  The interactive REPL (\texttt{biff} with no arguments) provides the primary
  terminal experience: readline (history, tab completion), real-time
  notifications (wall and message alerts while idle at the prompt), and modal
  talk with the \texttt{user:tty~\triangleright} prompt format.

  \texttt{/pair}---allowing a teammate to send input to your Claude Code
  session with explicit consent---is on the roadmap. It requires
  careful security design and is scoped to a future phase.
\end{faqpair}

\begin{faqpair}{Can agents use biff?}\label{faq:agents}
  Yes. Because biff is an MCP server, agents participate alongside humans
  as natural members of the team. An autonomous coding agent can
  \texttt{/plan} what it is working on, \texttt{/write} a human when it
  needs a decision, and show up in \texttt{/who} alongside everyone else.
  Each agent gets a distinct identity via \texttt{/tty}, targetable via
  \texttt{/write @user:tty}. \texttt{/who} shows host and directory per
  session so you can see where each agent is working. Workflow hooks
  automatically set plans and announce milestones.

  Biff is designed for hybrid teams --- humans and agents working together
  --- not for pure agent-to-agent coordination. If you need agents to
  coordinate autonomously without human involvement (shared memory,
  high-throughput task dispatch, swarm orchestration), dedicated tools like
  claude-flow and other agent frameworks are better suited for that. Biff
  serves the human-scale communication layer: the place where humans see
  what their agents are doing, and agents can reach a human when they need
  one. See \faqref{faq:agentic} for the broader coordination vision.
\end{faqpair}
...
\begin{faqpair}{What is the total addressable market?}\label{faq:tam}
  ...
  20 million cumulative signups by mid-2025 and 4.7 million paid
  subscribers by late 2025.\footnote{Microsoft Q2 FY2026 earnings;
  GitHub blog, ``Octoverse 2025.''} Claude Code launched in early 2025
  as the primary MCP client. The subset of engineers who
  ...
  multiplies; and as engineers run more agents alongside their own
  sessions, each human represents a growing number of sessions that benefit
  from biff's visibility and communication layer.
  See \faqref{faq:pnl} for steady-state economics.
\end{faqpair}
...
\begin{faqpair}{How will engineers discover biff?}\label{faq:discovery}
  Three channels, in order of expected impact:

  \begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Viral loop via committed config.} When a team member
    commits \texttt{.punt-labs/biff/config.yaml} to a shared repo, every engineer who
    clones that repo gets the team roster and relay configuration
    automatically --- the project carries its own communication context.
    The install path is two commands. This
    is the primary growth mechanism: one adopter seeds an entire team,
    and every clone of the repo is a distribution event.
    ...
    GitHub star, every fork, every config file committed is a
    distribution event.
  \end{enumerate}

  The real cost of acquisition is time, not money. Roughly 80\% of the
  go-to-market effort is founder time: writing blog posts, creating
  interest within personal networks, answering questions on forums,
  giving conference talks, and nurturing early adopters through the
  first-week onboarding. The remaining 20\% is modest spend on
  conference sponsorships and targeted ads. The product itself is free
  and the viral loop is built into the protocol (config commits
  with the repo), so the monetary CAC is near zero --- but the time
  investment to reach critical mass on each team is the true bottleneck.
\end{faqpair}
...
\begin{faqpair}{Who are the competitors and why will we win?}\label{faq:competitors}
  ...
  \textbf{Agent coordination frameworks} (claude-flow, hive-mind, swarm
  orchestrators) solve a different problem: pure agent-to-agent
  coordination without human involvement. They optimize for shared memory,
  high-throughput task dispatch, and machine-speed orchestration. Biff does
  not compete with these tools --- it complements them. A team might use
  claude-flow for agent-to-agent task routing and biff for the human-scale
  layer: seeing what agents are doing, messaging teammates, and having
  agents reach a human when they need one.
  ...
  where engineers already work, designed for hybrid teams of humans and
  agents. The moat is the MCP ecosystem: as more tools adopt MCP, biff's
  integration surface grows without additional development.
\end{faqpair}
...
\begin{faqpair}{What are the major technical risks?}\label{faq:tech-risks}
  \begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{User-visible notification gap (low risk, shipped).}
    Dynamic tool descriptions are visible to the AI model but
    not directly to the human user in the Claude Code UI. Biff ships
    status bar integration (\texttt{biff install} includes setup) showing
    \texttt{user:tty(N)} with three states: caught up, unreads (bold
    yellow), and messages off (\texttt{n}). Combined with model-level
    awareness via tool descriptions, the notification gap is addressed
    ...
    useful if your teammates also use it. Mitigation: the committed
    \texttt{.punt-labs/biff/config.yaml} means the second person on a team
    only needs to clone, run \texttt{uv tool install punt-biff} and
    \texttt{biff install} --- the configuration is already there. The demo relay eliminates
    infrastructure setup. The \texttt{/plan}, \texttt{/finger}, and
    \texttt{/who} commands provide value to individual users even without
    teammates. See \faqref{faq:discovery} for the distribution strategy.
  \end{enumerate}
\end{faqpair}

\begin{faqpair}{What dependencies exist on other teams or systems?}\label{faq:dependencies}
  Biff depends on the MCP protocol (maintained by Anthropic), Claude Code
  as the primary host client (also Anthropic), and NATS for the message
  relay. There are no
  dependencies on other punt-labs products. The relay server is a single
  NATS binary and can be self-hosted.
\end{faqpair}

\begin{faqpair}{Where does data live?}\label{faq:data}
  ...
  \texttt{/wall} broadcasts time-limited announcements to all terminals
  with duration-based expiry and poll-based cross-session refresh.

  Today, messages traverse the relay in cleartext.
  \texttt{.punt-labs/biff/config.yaml} in the repository contains the team
  member list and optional relay configuration. It is the root of trust:
  adding your username to it and committing it proves you have commit access to the
  repo, which is what makes you a team member. End-to-end encryption
  ships with biff 1.0 and will make the relay unable to read message
  contents. See \faqref{faq:timeline} for the encryption roadmap.
\end{faqpair}
...
\begin{faqpair}{What is the estimated development timeline?}\label{faq:timeline}
  \begin{description}[leftmargin=1.5em, labelwidth=10em, font=\bfseries]
    \item[Shipped] All twelve core commands: messaging (\texttt{/write},
    \texttt{/read}), presence (\texttt{/who}, \texttt{/finger},
    \texttt{/plan}), session identity (\texttt{/tty}), availability
    (\texttt{/mesg}), session history (\texttt{/last}), team broadcast
    (\texttt{/wall}), and real-time conversation (\texttt{/talk},
    \texttt{/talk\_listen}, \texttt{/talk\_end}). Cross-machine relay on
    Synadia Cloud with zero infrastructure setup. Per-project activation
    (\texttt{/biff y}) with idle connection management. Status bar
    integration showing unread counts. Workflow hooks for plan and wall
    automation. GitHub identity for display names. CLI parity with MCP
    tools (\texttt{biff who}, \texttt{biff finger}, etc.). Machine-readable
    output via \texttt{-{}-json} global flag. Library API
    (\texttt{from biff import commands}). Dev/prod plugin namespace
    isolation (\texttt{biff-dev} vs \texttt{biff}). Released and in daily
    use. See Appendix~A for the full command reference.
    \item[Future: Security] End-to-end encryption (NaCl/libsodium),
    GitHub identity and auth, per-repo NATS credentials.
    \item[Future: Real-Time] \texttt{/pair} for session sharing with
    explicit consent.
...
\begin{faqpair}{What is the agentic coordination vision?}\label{faq:agentic}
  Biff serves the human+agent collaboration layer, not the pure
  agent-to-agent coordination layer. These are different problems with
  different design constraints.

  Pure agent coordination --- shared memory, high-throughput task dispatch,
  swarm orchestration --- is well served by dedicated tools like claude-flow
  and other hive-mind frameworks. Those tools optimize for machine-speed
  coordination where no human is in the loop. Biff does not compete with
  them and is not trying to.

  What biff provides is visibility and communication for hybrid teams:
  humans and agents working together on the same codebase. Because biff is
  repo-scoped, you see only the humans and agents working on \emph{this}
  project --- not every team across your organization. Today, TTY
  sessions (\texttt{/tty}) give each agent a distinct identity --- one user
  running three agents shows three entries in \texttt{/who}, each targetable
  via \texttt{/write @user:tty}. Enriched presence shows host and directory
  per session so you can see where each agent is working. An agent can
  \texttt{/plan} what it is doing, \texttt{/write} a human when it needs a
  decision, and show up in \texttt{/who} alongside everyone else.
  \texttt{/last} provides an audit trail of all session activity --- who
  logged in, when, and for how long --- mirroring Unix \texttt{last(1)}. The next
  phase adds plan auto-expand (\texttt{/plan biff-bf8}) to surface what
  each agent is working on without anyone looking it up, and workflow hooks
  to automate coordination overhead: claiming a task auto-sets your plan,
  creating a PR triggers a \texttt{/wall} announcement.

  The design choice is intentional. Biff's Unix communication vocabulary
  --- \texttt{write}, \texttt{finger}, \texttt{who}, \texttt{wall},
  \texttt{mesg} --- is human-scale and purposeful by design. Pure agent
  swarms need shared memory and high-throughput coordination. Hybrid teams
  need human-scale communication with intent: the ability to see what your
  agents are doing, reach any of them directly, and have them reach you
  when it matters.
\end{faqpair}
...
\begin{faqpair}{What is the next step to validate the vision?}\label{faq:validate}
  ...
  See \faqref{faq:metrics} for the target numbers.
\end{faqpair}
...
\begin{faqpair}{What are the most likely ways biff fails?}\label{faq:failure-modes}
  Five scenarios, in descending order of likelihood:

  \begin{enumerate}[leftmargin=1.5em]
    \item \textbf{The anti-Slack niche is too small.} Engineers who hate
    Slack still use it because their PM is on it. Biff's repo-scoped,
    engineer-only model may not generate enough standalone value to
    overcome Slack's network effects. Kill signal: D30 retention below
    20\% across beta teams.

    \item \textbf{Claude Code ships native messaging.} Anthropic adds
    \texttt{/write} and \texttt{/who} to Claude Code directly, making
    biff redundant. Mitigation: biff is cross-client (any MCP host),
    cross-machine (NATS relay), and repo-scoped --- capabilities an
    embedded feature would not prioritize. But if Anthropic ships 80\%
    of biff's value natively, the remaining 20\% may not justify a
    separate install.

    \item \textbf{Time-to-critical-mass is too long.} Each team needs
    at least 2--3 members before biff provides value. If onboarding
    friction or the ``install biff, then convince a colleague'' loop
    takes weeks instead of days, teams abandon before reaching critical
    mass.

    \item \textbf{The relay becomes a liability.} A security incident or
    prolonged outage before the team is large enough to maintain the
    infrastructure erodes trust permanently. Today's cleartext-over-TLS
    model is honest but fragile --- a single relay compromise exposes
    every message.

    \item \textbf{Agents outgrow the communication model.} If
    engineering teams shift to fully autonomous agent fleets faster than
    expected, the hybrid human+agent model becomes a niche within a
    niche. Pure agent coordination tools (A2A, CrewAI, LangGraph) would
    serve the dominant use case, and biff's human-centric design becomes
    a liability rather than a differentiator.
  \end{enumerate}
\end{faqpair}
...
\riskitem{Value}{%
  ...
  The hybrid positioning --- designed for humans who work alongside agents,
  not for pure agent coordination --- is a strength: it keeps biff focused
  on its core audience (human engineers) while pure-agent tools serve a
  different need. The risk is that the hybrid framing is too narrow if the
  market shifts toward fully autonomous teams faster than expected, or too
  broad if engineers do not yet run enough agents for the hybrid story to
  resonate. The artisanal signal is positive: the tool is in daily use by
  its creators in a hybrid human+agent workflow. The remaining risk is
  whether that signal generalizes beyond the builders.}
...
\riskitem{Usability}{%
  ...
  Installation is three commands (\texttt{uv tool install punt-biff},
  \texttt{biff install}, \texttt{biff doctor}). Time-to-value is under five
  minutes. The primary usability risk is discoverability: new users need to
  learn the command vocabulary. Mitigation: the vocabulary is small (12
  shipped commands), the names are intuitive (borrowed from Unix), and the
  tool descriptions are self-documenting within Claude Code. The secondary
  ...
  Mitigation: the committed-config viral loop means most users arrive
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
  ...
  Read a user's plan, current status, host, and directory.
  Non-intrusive --- the target is not notified. \\

  \texttt{/who} & BSD \texttt{who} &
  List all active sessions with 7 columns: NAME, TTY, IDLE, S (message
  status), HOST, DIR, and PLAN. Shows who is online, where they are
  ...
  \texttt{/tty ``name''} & BSD \texttt{tty} &
  Name the current session. Each agent gets a distinct identity ---
  one user with 3 sessions shows 3 entries in \texttt{/who}, targetable
  via \texttt{/write @user:tty}. \\

  \texttt{/mesg y} | \texttt{n} & BSD \texttt{mesg} &
  Control whether you receive messages. When off (\texttt{n}), incoming
  messages are queued until you turn it back on (\texttt{y}), and the
  status bar shows \texttt{(n)} instead of the unread count. \\

  \texttt{/last} & BSD \texttt{last} &
  Show session login/logout history. Displays who logged in, from where,
  when, and for how long. Active sessions are marked ``still logged in''.
  Three-layer logout detection: sentinel-based (graceful shutdown),
  orphan detection (crash recovery), and KV watcher (TTL expiry). \\

  \texttt{/wall ``text''} & BSD \texttt{wall} &
  Broadcast a time-sensitive announcement to all terminals. Visible
  immediately on the status bar with duration-based expiry (default 1h,
  max 3d). Does not go to inboxes --- it is a banner, not a message.
  Cross-session poll-based refresh. \\

  \texttt{/biff enable} | \texttt{disable} & BSD \texttt{biff} &
  Per-project activation toggle. \texttt{/biff enable} (or \texttt{biff enable}
  from CLI) opts in by writing the committed \texttt{.punt-labs/biff/enabled}
  marker; \texttt{/biff disable} opts out and removes it.
  Dormant mode: no NATS connection, no consumers, no status line until
  opted in. Persistent NATS connection with nap mode: after 2 minutes idle,
  polling reduces to 30s but connection stays alive for KV watches and
  subscriptions (DES-019). \\

  \texttt{/talk @user ``msg''} & BSD \texttt{talk} &
  Two-phase handshake: inviter waits, target accepts. Messages via
  NATS core pub/sub (ephemeral, no inbox). Mutual hangup: either side
  types \texttt{end} and both return to REPL. \\

  Interactive REPL & --- &
  \texttt{biff} with no arguments launches an interactive session with
  readline (history, tab completion), real-time notifications (wall and
  message alerts while idle), and modal talk. Proper session lifecycle:
  login/logout events, heartbeat, KV presence, clean cleanup. \\

  Workflow hooks & --- &
  Claiming a bead suggests \texttt{/plan}. Creating or merging a PR
  suggests \texttt{/wall} announcement. Hooks gate on the committed
  \texttt{.punt-labs/biff/enabled} marker --- silent when biff is not enabled. \\

  CLI parity & --- &
  Every MCP tool is also a CLI command: \texttt{biff who},
  \texttt{biff finger}, \texttt{biff write}, etc. Enables scripting and
  use outside Claude Code sessions. \\

  \texttt{-{}-json} global flag & --- &
  Machine-readable JSON output for all commands. Enables programmatic
  consumption by scripts and other tools. \\

  Library API & --- &
  \texttt{from biff import commands, CliContext, CommandResult}. Use biff
  as a Python library in your own tools --- same logic, no subprocess. \\

  Dev/prod isolation & --- &
  Plugin uses \texttt{biff-dev} namespace during development; release
  scripts swap to \texttt{biff} for production. Both can run side-by-side
...
\begin{tabularx}{\textwidth}{@{}l X@{}}
  ...
  messaging (\texttt{/write}) and broadcast (\texttt{/wall})
