...
\newcommand{\prfaqcurrentversion}{\prfaqversion{2}{4}}
...
other's files. Each agent gets a distinct identity via \texttt{/tty} sessions,
and \texttt{/who} shows host and directory per session so you can see when
agents share a filesystem. Workflow hooks tie into your development process:
claiming a task auto-sets your plan, creating a PR triggers a \texttt{/wall}
...
\begin{faqpair}{Can agents use biff?}
  ...
  and show up in \texttt{/who} alongside everyone else. Each agent gets a
  distinct identity via \texttt{/tty}, targetable via
  \texttt{/write @user:tty}. \texttt{/who} shows host and directory per
  session, so you can see when agents share a filesystem and need to
  coordinate. Workflow hooks automatically set plans and announce
  milestones. Biff does not distinguish between human and agent sessions.
  If it speaks MCP, it can participate.
\end{faqpair}
...
\begin{faqpair}{What is the estimated development timeline?}
  \begin{description}[leftmargin=1.5em, labelwidth=10em, font=\bfseries]
    ...
    \item[Next: Agentic] \texttt{/wall} (team broadcast).
...
\begin{faqpair}{What is the agentic coordination vision?}
  ...
  This is already shipping. TTY sessions (\texttt{/tty}) give each agent a
  ...
  a filesystem and need to coordinate to avoid file conflicts. The next
