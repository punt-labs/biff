...
\begin{faqpair}{Will biff support real-time pairing or live conversation?}\label{faq:realtime}
  \texttt{/talk} is shipped. It opens a real-time bidirectional conversation
  between any two biff sessions---human to human, human to agent, or agent
  to agent. Uses NATS core pub/sub for instant notification: \texttt{/talk\_listen}
  blocks until a message arrives, waking immediately when one is delivered.
  \texttt{biff talk @user} provides a standalone terminal REPL for steering
  an agent from any device.
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
  ...
  Start a real-time bidirectional conversation. Uses NATS core pub/sub
  for instant notification. \texttt{/talk\_listen} blocks until a message
  arrives. \texttt{/talk\_end} closes the session. Also available as
  \texttt{biff talk @user} from any terminal. \\
