...
\begin{faqpair}{What is the estimated development timeline?}
  \begin{description}[leftmargin=1.5em, labelwidth=10em, font=\bfseries]
    ...
    \texttt{/who}, \texttt{/plan}, \texttt{/finger}, \texttt{/tty},
    \texttt{/mesg y/n}, and \texttt{/last}. NATS-based relay for
    cross-machine messaging on Synadia Cloud. POP-model store-and-forward
    for \texttt{/write}. TTY sessions (each agent gets a distinct identity,
    targetable via \texttt{/write @user:tty}). Enriched presence
    (\texttt{/who} shows 7 columns: NAME, TTY, IDLE, S, HOST, DIR, PLAN;
    \texttt{/finger} shows host and directory per session). Session
    history (\texttt{/last} shows login/logout records modeled after Unix
    \texttt{last(1)}, with three-layer logout detection). Per-session status
    bar showing \texttt{user:tty(N)} with three states: caught up, unreads
    (bold yellow), and messages off (\texttt{n}). \texttt{/mesg n}
    suppresses the unread count on the status line.
    Display names from GitHub identity. Team broadcast (\texttt{/wall})
    with duration-based expiry (default 1h, max 3d), poll-based cross-session
    refresh, and status bar integration. Per-project activation via
    \texttt{/biff y} (or \texttt{biff enable} from CLI) with dormant mode ---
    no NATS connection, no consumers, no status line until opted in. Lazy
    NATS connection management releases TCP after 5 minutes idle and
    reconnects briefly every 10 minutes (POP-mode polling). Workflow hooks:
    claiming a bead suggests \texttt{/plan}, creating or merging a PR
    suggests \texttt{/wall}. Released and in use.
...
\begin{faqpair}{What are the most likely ways biff fails?}
