...
\texttt{pip install biff}, then \texttt{biff install} to register the MCP
...
Install with \texttt{pip install biff}, then run \texttt{biff install}
...
\begin{faqpair}{What is biff and who is it for?}\label{faq:what-is-biff}
  ...
  biff is for you. It provides eight slash commands covering messaging,
...
\begin{faqpair}{How is biff different from Slack?}\label{faq:diff-slack}
  ...
  And because team configuration lives in a \texttt{.biff} file committed to
  the repo, communication is scoped to the project you are working on right
...
\begin{faqpair}{How do I get started?}\label{faq:get-started}
  Run \texttt{pip install biff}, then \texttt{biff install} to register
...
\begin{faqpair}{How will engineers discover biff?}\label{faq:discovery}
  ...
  and the viral loop is built into the protocol (\texttt{.biff} commits
...
\begin{faqpair}{What are the major technical risks?}\label{faq:tech-risks}
  \begin{enumerate}[leftmargin=1.5em]
    ...
    needs to clone, run \texttt{uv tool install --editable .} and
...
\begin{faqpair}{Where does data live?}\label{faq:data}
  ...
  Today, messages traverse the relay in cleartext. The \texttt{.biff}
  file in the repository contains the team member list and optional relay
  configuration. It is the root of trust: adding your username to
  \texttt{.biff} and committing it proves you have commit access to the
...
\riskitem{Usability}{%
  ...
  Installation is four commands (\texttt{git clone},
  \texttt{uv tool install --editable .}, \texttt{biff install},
  \texttt{biff doctor}). Time-to-value is under five minutes. The
  primary usability risk is discoverability: new users need to learn the
  command vocabulary. Mitigation: the vocabulary is small (8 shipped
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
  ...
  from CLI) opts in via \texttt{.biff.local}; \texttt{/biff n} opts out.
  ...
  suggests \texttt{/wall} announcement. Hooks gate on \texttt{.biff.local}
  --- silent when biff is not enabled. \\
