...
\begin{faqpair}{What is the estimated development timeline?}
  \begin{description}[leftmargin=1.5em, labelwidth=10em, font=\bfseries]
    ...
    TTY sessions (each agent gets a distinct identity, targetable via
    \texttt{/write @user:tty}). Enriched presence (\texttt{/who} shows
    7 columns: NAME, TTY, IDLE, S, HOST, DIR, PLAN; \texttt{/finger}
    shows host and directory per session). Per-session status bar
    showing \texttt{user:tty(N)} with three states: caught up, unreads
...
\begin{faqpair}{What is the agentic coordination vision?}
  ...
  decision, and show up in \texttt{/who} alongside everyone else. The next
...
\riskitem{Usability}{%
  ...
  command vocabulary. Mitigation: the vocabulary is small (7 shipped
