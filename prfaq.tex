...
when to engage. When you need deeper
collaboration, \texttt{/pair @kai} invites a teammate to send input to your
Claude Code session --- and they can only join when you explicitly consent.
When a problem needs three people for thirty minutes, \texttt{/hive} creates a
temporary group that dissolves when the work is done. The full command
reference, organized by priority tier, is in Appendix~A.
...
team member list. When you clone a repo that has a \texttt{.biff} file,
biff automatically connects you to the right team. Encryption keys are
generated silently on first use --- there is no account to create, no
workspace to configure, no keys to manage, and no browser to open. Type
\texttt{/who} to see your teammates. Within five minutes of installation, you
can send your first encrypted message, see your team's presence, and set
your plan.
...
Install with \texttt{pip install biff-mcp}. The core tool --- including
end-to-end encryption --- is free and always will be. The hosted relay is
free for teams of up to five. For larger teams that need uptime guarantees,
audit logs, and admin controls, biff teams will be available in Q3 2026.
...
\begin{faqpair}{How do I get started?}
  ...
  automatically. Encryption keys are generated and registered
  transparently --- you never see them. Type \texttt{/who} to see your
  team. No account creation, no workspace to configure. You are
  communicating securely within five minutes.
\end{faqpair}

\begin{faqpair}{What happens to my messages? Is communication private?}
  All messages are end-to-end encrypted. Direct messages (\texttt{/mesg})
  are encrypted with the recipient's public key --- the relay stores
  ciphertext it cannot read. Team messages (\texttt{/wall}, \texttt{/plan})
  use a shared team key. Encryption is automatic and invisible: keypairs
  are generated silently on first use, and the relay manages key
  distribution. The relay follows a POP mail model: messages are held
  only until the recipient reads them, then discarded. Biff is not a
  system of record. In the self-hosted mode, messages route through your
  own infrastructure and nothing touches punt-labs. Biff never reads,
  analyzes, or trains on your messages.
\end{faqpair}
...
\begin{faqpair}{What are the major technical risks?}
  \begin{enumerate}[leftmargin=1.5em]
    \item \textbf{MCP notification mechanism (low risk, validated).} A
    spike validated that Claude Code acts on
    \texttt{notifications/tools/list\_changed} sent over SSE: when the
    biff server updates a tool description (e.g., ``Check messages (1
    unread: @kai about auth)''), the model sees the change and can
    proactively mention it to the user. The full chain --- message
    arrival, tool re-registration, SSE notification, model awareness ---
    has been tested end-to-end. The remaining MCP spec maturity risk is
    mitigated by wrapping all MCP interactions behind an internal
    abstraction layer.

    \item \textbf{User-visible notification gap (medium risk).} Dynamic
    tool descriptions are visible to the AI model but not directly to the
    human user in the Claude Code UI. There is no badge, popup, or
    inline notification for the user. Mitigation: the terminal status bar
    is a configurable shell command that can read local files. Biff will
    write unread state to a file, and the status bar script displays a
    live count. This is a proven pattern but requires per-terminal setup.
    ...
    The relay follows a POP mail model: messages are stored on the relay
    only until the recipient reads them, then discarded. Biff is not a
    system of record. The relay is a NATS JetStream instance hosted by
    punt-labs; teams that want data sovereignty can self-host the same
    ...
    interactions logged, session owner retains kill switch. This is scoped
    to Phase~3, giving time for the security model to mature.
...
\begin{faqpair}{Where does data live?}
  All data lives on the relay, encrypted. The relay serves three
  durability tiers:

  \textbf{Persistent}: \texttt{/plan} status is stored on the relay,
  encrypted with the team key. Like the original BSD
  \texttt{\textasciitilde/.plan} file, it survives sessions, disconnects,
  and restarts. \texttt{/finger} reads the persistent plan and combines
  it with live presence.

  \textbf{Store-and-forward}: \texttt{/mesg} follows a POP mail model
  --- messages are encrypted with the recipient's public key, held on
  the relay until read, then discarded. The relay cannot read them.

  \textbf{Ephemeral}: \texttt{/wall} is a broadcast --- if you are not
  connected when it happens, you miss it, just like BSD \texttt{wall}.
  \texttt{/who} and \texttt{/biff on/off} are live presence state.
  \texttt{/hive}, \texttt{/talk}, and \texttt{/pair} are sessions that
  exist only for their duration.

  The \texttt{.biff} file in the repository contains the relay URL and
  team member list. It is the root of trust: adding your username to
  \texttt{.biff} and committing it proves you have commit access to the
  repo, which is what makes you a team member.
\end{faqpair}

\begin{faqpair}{What is the estimated development timeline?}
  A technical spike has validated the core architecture: HTTP transport with
  SSE for server-push notifications, dynamic tool descriptions via
  \texttt{notifications/tools/list\_changed}, and end-to-end message
  awareness in Claude Code. The architecture is proven; what remains is
  implementation.
  ...
  \begin{description}[leftmargin=1.5em, labelwidth=6em, font=\bfseries]
    ...
    \texttt{/plan} and session awareness. Architecture validated by spike.
    \item[Phase 2 (4 weeks)] NATS-based relay for cross-machine messaging.
    POP-model store-and-forward for \texttt{/mesg}. Ephemeral broadcast
    for \texttt{/wall}. \texttt{/hive}, \texttt{/send}, \texttt{/cr}.
    Team communication functional end-to-end.
    \item[Phase 3 (4 weeks)] \texttt{/talk} real-time conversation,
    \texttt{/pair} with permission model. Security audit of the
    encryption and permission models. Note: the spike found that direct
    subprocess handoff is not viable in MCP; \texttt{/talk} will use
    model-mediated message exchange.
...
\begin{faqpair}{What is the revenue model?}
  Open core. The biff CLI tool, self-hosted relay, and end-to-end
  encryption are free and open source (MIT license). Revenue comes from
  \textbf{biff teams}, a hosted relay service priced at
  \$10/user/month for teams above the free tier:

  \begin{itemize}[leftmargin=1.5em]
    \item Free tier: up to 5 team members on the hosted relay
  ...
  Encryption is not a paid feature --- it is a baseline for every user.
...
\riskitem{Feasibility}{%
  \textbf{Low risk (validated).} A technical spike has confirmed the core
  architecture end-to-end: HTTP transport with SSE, dynamic tool
  descriptions via \texttt{notifications/tools/list\_changed}, and
  model-visible message awareness in Claude Code. The MCP notification
  mechanism --- the highest-uncertainty component --- works as designed.
  The remaining technology is well-understood: NATS JetStream for the
  message relay, NaCl/libsodium (Curve25519 + XSalsa20-Poly1305) for
  end-to-end encryption, and PyNaCl for the Python implementation. These
  are the same cryptographic primitives used by Signal, WireGuard, and
  age. The highest-risk component is the \texttt{/pair} feature, which
  requires careful permission design for session input sharing. This is
  scoped to Phase~3, giving time for the security model to mature. No
  component requires novel technology.}
...
Commands are organized by priority tier using the MoSCoW framework.
\textbf{Must Have} commands ship in Phase~1 and deliver standalone value.
\textbf{Should Have} commands ship in Phase~2 and enable team workflows.
\textbf{Could Have} commands ship in Phase~3 and require additional
architecture work identified during technical spikes.

\subsection*{Must Have --- Phase 1: Core Communication}
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
  ...
  it when they choose. Core validated by technical spike. \\
...
\subsection*{Should Have --- Phase 2: Team Communication}
...
\subsection*{Could Have --- Phase 3: Real-Time \& Pairing}
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
  ...
  Open a real-time bidirectional conversation. The technical spike found
  that direct subprocess handoff is not viable in MCP; this command will
  use model-mediated message exchange instead. \\
  ...
  control. Highest security design complexity. \\
