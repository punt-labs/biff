...
\begin{faqpair}{What is the estimated development timeline?}\label{faq:timeline}
  \begin{description}[leftmargin=1.5em, labelwidth=10em, font=\bfseries]
    \item[Shipped] All nine core commands: messaging (\texttt{/write},
    \texttt{/read}), presence (\texttt{/who}, \texttt{/finger},
    \texttt{/plan}), session identity (\texttt{/tty}), availability
    (\texttt{/mesg}), session history (\texttt{/last}), and team broadcast
    (\texttt{/wall}). Cross-machine relay on Synadia Cloud with zero
    infrastructure setup. Per-project activation (\texttt{/biff y}) with
    idle connection management. Status bar integration showing unread
    counts. Workflow hooks for plan and wall automation. GitHub identity
    for display names. Released and in daily use.
    See Appendix~A for the full command reference.
    \item[Next: Agentic Coordination] Plan auto-expand (\texttt{/plan
    biff-bf8} shows the task title).
