...
\begin{faqpair}{What is the estimated development timeline?}
  ...
  \begin{description}[leftmargin=1.5em, labelwidth=6em, font=\bfseries]
    ...
    audit. Note: the spike found that direct subprocess handoff is not
    viable in MCP; \texttt{/talk} will use model-mediated message
    exchange.
...
\riskitem{Feasibility}{%
  ...
  The remaining technology (WebSocket relay, message routing, file-based
  storage) is well-understood. The highest-risk component is the
  \texttt{/pair} feature, which requires careful permission design for
  session input sharing. This is scoped to Phase~3, giving time for the
  security model to mature. No component requires novel technology.}
