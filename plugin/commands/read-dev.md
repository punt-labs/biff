...
Call `mcp__plugin_biff-dev_tty__read_messages` with no arguments. The tool retries a transport error once internally, per inbox, and never raises for a transport failure — every outcome below is a normal return value to check, never an error to catch.
