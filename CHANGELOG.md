...
- **`set_poll_interval`'s description and disable-response text claimed push detection survives regardless of relay backend, but `_relay_pushes_inbox_notify()` is `NatsRelay`-only.** A filesystem-backed `LocalRelay` has no push mechanism at all, so disabling the interval there silently kills detection entirely rather than losing a "periodic safety net." Both texts now branch on the actual relay backend.
