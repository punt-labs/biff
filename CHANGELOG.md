...
- **The deprecated repo-root config files `.biff`, `.biff.local`, `.ethos/missions.jsonl`, and `.lux/config.md` (this repo's own copies, not the read paths).** `.biff`/`.biff.local` predate the `.punt-labs/biff/config.yaml`/`config.local.yaml` config location and are read by nothing — `is_enabled()` and the config loader only ever look under `.punt-labs/biff/`. `.ethos/missions.jsonl` predates `.punt-labs/ethos.yaml` + `.punt-labs/ethos/identities/`, which is what `resolve_agent_identity_from_disk()` and the ethos CLI integration actually read; nothing in this repo reads bare `.ethos/`. `.lux/config.md` is different: `is_lux_enabled()` in `src/biff/_stdlib.py` still reads exactly that path today, so removing this repo's own file turns the lux beads-progress panel off for this repo (operator-confirmed trade-off) rather than removing dead code — no source change accompanies this entry.
