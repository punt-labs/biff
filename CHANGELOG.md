...
- **A runnable push-vs-poll demonstration proves the biff-5ex latency claim** (`scripts/demo-push-vs-poll.sh`). An A/B comparison on the sender side only — one arm bypasses `deliver()`'s wake poke (the pre-biff-5ex broadcast shape), the other uses the real `deliver()` — against the same, honest receiving MCP server. `tests/test_nats_e2e/test_push_vs_poll_latency.py` (tier 3c, local `nats-server`) prints a side-by-side latency table and also demonstrates the companion load claim (steady-state `get_unread_summary` call volume with no arrivals); `tests/test_relay_image/test_push_demo.py` (tier 3b) runs the same comparison against the real Docker relay image and saves a transcript. `NotificationTracker` (`src/biff/testing`) now records a monotonic timestamp per `tools/list_changed` notification, not just a count.
- **A new `unread-nudge.sh` hook delivers unread-mail context to the model directly, instead of relying on a tool description the model's harness view never refreshes** (biff-5ex canary follow-up). Wired into `UserPromptSubmit` and `PostToolUse` (match-all), it reads the per-session unread file directly — a pure-shell reimplementation of `session_key.py`'s PID-walk, no Python startup, to stay inside `UserPromptSubmit`'s <100ms budget — and injects an `additionalContext` nudge ("You have N unread biff messages — call read_messages now") the moment the count rises, tracking the last-nudged count in a sibling `.nudged` file so it fires once per change rather than once per tool call. Covers active-session delivery; `/biff:read`'s cron loop still covers the idle gap where no prompt or tool call fires the hook.
...
- **`_publish_talk_notification` and the new inbox-notify poke publish are rebuild-robust** (biff-1f2): both now resolve the NATS client via a shared `_live_nc_or_reconnect()` helper — reuse the cached client when it's still live, reconnect through `get_nc()`/`_ensure_connected()` only when it was actually discarded by a wedge teardown — instead of reading the cached client reference directly and silently no-opping on `None`. `get_wall()`'s expiry cleanup likewise re-resolves the KV handle immediately before its delete, closing the same class of staleness across the one `await` that separates the fetch from the delete.

### Fixed

- **`/biff:read`'s no-arg check could sit on an unread message for minutes despite the status bar showing it immediately** (biff-5ex canary). The check gated on the `read_messages` tool description's `(N unread)` marker, but the harness's tool list is a session-start snapshot — a mid-session `tools/list_changed` refresh reaches Claude Code (confirmed via `ListToolsRequest` in the logs) but never reaches the model's view of the description, so the marker this command looked for was invisible until the model's next fresh turn. Mail now gates on `biff statusline`'s ground-truth per-session unread count instead — a different code path (the on-disk file DES-011a's PPID-keyed session key resolves) that isn't subject to the same staleness. Talk keeps its description-based gate (no equivalent persistent ground-truth count exists for it) plus a new best-effort peek at the ephemeral talk item on statusline line 2.
...
- **A targeted (`user:tty`) message's wake poke never marked the inbox poke gate, deferring its unread-count detection to the backstop.** The gate-marking split into two SUBs by kind (talk vs. inbox-notify), but a targeted message rides its wake poke on the talk-notify subject, not the inbox-notify one — the talk SUB's callback only woke the poller (`state.activity.wake()`), never the gate `_active_tick` actually reads. The callback now classifies each frame via `TalkNotification.is_wake_poke` and marks the same gate for a wake poke specifically, restoring targeted-message detection to push latency.
- **A dual session's companion identity had no inbox-notify subscription, so a broadcast addressed to the companion had zero push signal.** `poll_inbox` now opens a second, independent inbox-notify SUB bound to `state.companion.user` whenever a companion is set, reusing `subscribe_inbox_notify`/`_reconcile_inbox_notify_sub` with a `user` parameter rather than adding a companion-specific code path. `docs/nats-relay.tex`'s `SubKind` free type gained a third constructor, `inboxNotifyCompanion`, re-verified with fuzz and a per-kind ProB CTL liveness check.
- **`set_poll_interval n` silently disabled ALL push detection, contradicting its own description.** The poller task hosting the always-on SUBs was only created when `poll_interval > 0`; it now always runs, with a shared `asyncio.Event` making push-to-refresh latency genuinely interval-independent (including at `interval <= 0`, where a bounded 30s liveness fallback wakes the loop for subscription reconcile only — a bare fallback timeout never runs the periodic tick work). Only the periodic work with no push signal of its own — wall-countdown re-render, stale-invite expiry, the unread backstop — actually degrades when disabled; the tool text now says so honestly, including the real 15x backstop-to-interval ratio.
- **A poke consumed by the gate just before a transient relay failure could freeze the unread count until the next backstop tick.** `_InboxPokeGate`'s `claim()` now atomically clears the poke and resets the backstop clock before the caller's refresh runs (so a poke arriving mid-refresh survives to the next tick), and `refresh_read_messages` reports fetch success/failure back to its caller so a failed fetch re-marks the gate and retries on the very next tick instead of waiting out a full backstop.
- **A best-effort wake-poke publish could block on an unbounded reconnect+re-provision cycle** (up to `_CONNECT_PROVISION_TIMEOUT`, ~20s, plus the dial) inside `_live_nc_or_reconnect`'s fall-through, contradicting its own "never blocks" documentation. Now bounded by `asyncio.timeout(_NOTIFY_RECONNECT_TIMEOUT)` (3s).
- **A poke fired after `close()` could re-dial and re-provision a connection nothing would ever close again.** `NatsRelay` gained a terminal `_closed` flag, set only by `close()` (never the reversible `disconnect()`), checked by `_live_nc_or_reconnect` before any reconnect attempt.
- **`_validate_user` did not actually reject `:`, despite `inbox_notify_subject`'s disjointness-from-`talk_notify_subject` docstring claiming it did.** A colon-bearing username could in principle collide a broadcast poke subject with a session's talk-notify subject. `_validate_user` now rejects `:`, matching `_validate_tty`'s existing rejected set.
- **`_open_connection`'s cleanup on a failed/cancelled provision only caught `Exception`, so an outer cancellation (e.g. `_live_nc_or_reconnect`'s bounded fall-through) landing mid-provision skipped `safe_close()` entirely** — `asyncio.CancelledError` is a `BaseException`, not an `Exception`, so it fell straight through and leaked the freshly-dialed client (neither closed nor installed). The cleanup now also catches `CancelledError`.
- **`disconnect()` never took the connection lifecycle lock, unlike every other lifecycle transition** (`_ensure_connected`, `_force_reconnect`, `close`). A `disconnect()` racing a blocked reconnect could observe nothing to tear down and return, while the in-flight reconnect finished moments later and silently reconnected the relay right after `disconnect()` reported it torn down. `disconnect()` now serialises on the same `_connect_lock`.
- **The talk-SUB callback stringified every decoded JSON value before `TalkNotification.from_payload`'s wire-boundary type guard could see the original type, defeating the guard entirely** — see Security, below.
- **`poll_inbox`'s two initial always-on subscribes ran before the `try`/`finally` that unsubscribes on exit.** A cancellation landing between `subscribe_talk()` returning and `subscribe_inbox_notify()`'s own await escaped before either binding variable was ever assigned inside the protected block, so the finally's teardown loop never saw the already-established talk SUB and it leaked. Both initial subscribes now run inside the `try`.
- **The companion talk-notify callback marked the shared inbox poke gate for every frame, including genuine talk messages/invites/end/withdraw addressed to the companion** — forcing a spurious `stream_info` recompute on every real conversation turn with the companion. It now classifies the payload as a wake poke first, mirroring `subscribe_talk`'s own discipline.
- **`subscribe_companion_talk` read `state.companion` twice with an `await` in between, so a heartbeat-loop rollback to `None` mid-flight could raise an uncaught `AttributeError` and kill the whole `poll_inbox` task.** It now takes the companion session as a caller-supplied parameter; `_reconcile_companion_subs` snapshots `state.companion` exactly once per reconciliation pass and threads that value through both the inbox and talk sub reconciles.
- **A newly (re)established companion inbox-notify subscription could never see broadcast mail that arrived before it existed** — core NATS has no replay, so pre-existing mail sent to a companion identity that appeared mid-session (the common production case) stayed undetected at a disabled poll interval. `_reconcile_companion_subs` now marks the poke gate when the companion's inbox-notify SUB is newly bound (first bind, or a retry that finally succeeds), forcing the next tick's real `stream_info` recompute to catch up — mirroring the initial `True` `_InboxPokeGate.__init__` already gives this session's own inbox at poller startup.
- **A `wake_event.set()` landing in the narrow window between `asyncio.wait()`'s internal timeout decision and `_sleep_or_wake`'s own `wake_event.clear()` was silently erased**, reporting TIMEOUT instead of EVENT — at a disabled poll interval, `poll_inbox` treats TIMEOUT as "skip this tick's recompute," so the lost poke stayed lost until the next fallback tick rather than merely delayed. `_sleep_or_wake` now snapshots `wake_event.is_set()` before clearing it and ORs that into the EVENT/TIMEOUT decision.
- **`set_poll_interval`'s description and disable-response text claimed push detection survives regardless of relay backend, but `_relay_pushes_inbox_notify()` is `NatsRelay`-only.** A filesystem-backed `LocalRelay` has no push mechanism at all. Both texts now branch on the actual relay backend (further corrected below: disabling no longer kills detection on `LocalRelay` either — it falls back to a fixed ~30s poll).
- **`plugin/hooks/unread-nudge.sh`'s process-tree walk used `declare -A` (bash 4+), which macOS's system `/bin/bash` (3.2, GPLv2-frozen) does not support** — the walk errored out silently there, every session fell back to `$PPID`, and the nudge was effectively disabled on macOS. The walk now runs entirely in `awk` (portable, POSIX-required, has always had associative arrays); bash only captures its single-line result.
- **`unread-nudge.sh`'s `.nudged` sidecar could outlive its `.json` file.** The server's shutdown paths removed only the unread-status JSON, so a PID reused by an unrelated later session could inherit a stale sidecar whose leftover count happened to match the new session's own first real count, silently suppressing the nudge that should fire for it. Both shutdown cleanup sites now go through a new `_remove_unread_files()` helper that deletes both files together; the hook also self-heals defensively (a sidecar with no matching JSON is always stale).
- **`/biff:read`'s documented statusline parse coerced a mesg-off session's hidden count to `0`, silently stopping mail delivery for it.** `biff statusline` prints the literal letter `(n)` — not a number — when mesg is off, regardless of the real count; the parse-failure fallback treated that non-numeric shape the same as a genuine parse error and skipped pulling mail, while `unread-nudge.sh` (the other delivery path) reads the on-disk count directly and keeps nudging regardless of mesg. The mail gate now recognizes `(n)` as "count unknown, possibly nonzero" and pulls. `read-dev.md` kept in sync.
- **A hard SIGTERM/SIGINT/SIGHUP kill never removed the unread-status JSON or its `unread-nudge.sh` `.nudged` sidecar — only the normal (non-signal) lifespan-teardown path called `_remove_unread_files`.** A PID reused by an unrelated later session could inherit a stale sidecar whose leftover count happened to match its own first real count, silently suppressing the nudge that should fire for it. The signal handler's cleanup steps now include unread-file removal. As defense in depth for the unrecoverable SIGKILL case (nothing can catch it), the unread-status JSON also now carries the writing process's own `server_pid`, and `unread-nudge.sh` distrusts any `.nudged` sidecar stamped with a different `server_pid` than the current JSON — a same-PID subprocess restart also resets it, trading one redundant re-nudge for never silently suppressing a real one.
- **`unread-nudge.sh` nudged even when the session had run `/mesg off`, bypassing the documented mute toggle.** The hook now reads `biff_enabled` from the unread-status JSON and stays silent while muted — consistent with the statusline, which also hides the count in that state. `/biff:read`'s deliberate pull (not a proactive nudge) is intentionally unaffected; the two paths' asymmetry is now documented in both places.
- **A stranded SUB's successful resubscribe (any always-on kind: talk, inbox-notify, or either companion SUB) never rearmed the poke gate, so a message published while the SUB was orphaned had no way to ever get detected.** `_reconcile_always_on_sub` — shared by every SUB kind — now marks the poke gate and wakes the poller whenever it establishes a fresh binding (first bind, a retry after failure, or a wedge-teardown resubscribe), not just on the companion's first bind as the prior fix round covered. The now-redundant per-call-site check in `_reconcile_companion_subs` was removed in favor of this single shared invariant: every event that can change unread state either marks the gate directly or is covered by the backstop's periodic fallback tick.
- **A disabled poll interval (`n`) on a filesystem-backed `LocalRelay` disabled `/write` detection for the rest of the session's life, not just delayed it.** The disabled-interval tick-work skip assumed the relay's always-on SUBs (or their poke-driven wake) would eventually trigger a recompute — true only for a `NatsRelay`. `LocalRelay` has no push mechanism of any kind, so nothing else would ever set `wake_event` again. The skip is now conditioned on `_relay_pushes_inbox_notify()`; a non-push relay always runs full tick work on the disabled-interval fallback tick instead.
- **A dual session's companion identity had no way to change.** `_CompanionSubs` retained live SUB handles but not which companion identity they were bound to, so `state.companion` rolling back to `None` (or changing to a different companion) with the connection generation unchanged left the reconcile with nothing to notice — the generation-only check saw an already-live SUB and did nothing, keeping this process subscribed to the *old* identity's subjects forever and never binding the *new* identity's subjects at all. `_CompanionSubs` now tracks the bound identity; any mismatch unsubscribes the stale SUBs and starts clean.
- **The mesg-off nudge suppression returned before ever touching the `.nudged` sidecar, so a stamp written before muting survived the entire muted period untouched.** If the count changed one or more times while muted (mail read to zero, new mail arriving) and happened to land back on that same stale value by the time mesg came back on, the rising-change gate compared equal and silently swallowed a nudge for genuinely unread mail the user was never told about. The muted path now clears the sidecar unconditionally, not only at count `0`, so the first post-unmute tick always starts from a clean "never nudged" baseline — a redundant nudge in the rare case the count truly never changed, never a silently suppressed one.
- **Dropping the companion (rolling `state.companion` back to `None`) unsubscribed its SUBs but never marked the poke gate.** The companion's contribution to the combined unread count vanishes from the next recompute the instant it drops — itself an unread-state change — but with no SUB left to (re)bind this pass, nothing marked the gate on its behalf; at a disabled poll interval the stale, still-companion-inclusive total would persist indefinitely. The drop path now marks the gate and wakes the poller directly. (Verified the sibling case — the companion *changing* to a different identity, not dropping — was already covered: it falls through to the normal fresh-bind path, which already marks the gate on success.)

### Security

- **A forged wire payload could render arbitrary attacker-controlled JSON as if it were literal message text.** The talk-SUB callback's dict comprehension ran `str(v)` on every decoded JSON value before `TalkNotification.from_payload`'s `_trusted()` type guard ever saw it, so by the time the guard checked `isinstance(value, str)` every value already *was* a string — the guard against non-`str` JSON types (forged `null`, numbers, nested structures) was unreachable. A forged `{"body": null}` became `str(None)` == `"None"`, passing the (defeated) guard and rendering as if the sender had actually typed the word "None". Only the dict keys are stringified now (JSON object keys always are); values pass through untouched so `_trusted()` actually discriminates.
...
- **Dropped mid-session `tools/list_changed` notification had no recovery path** (biff-ue2). `notify_tool_list_changed()`'s suspenders send-failure path cleared the dead session reference but never recorded the drop, so once a description had already changed, the change-gate suppressed every later re-notify attempt and the client's cached tool list went stale for the rest of the session. A suspenders failure now sets `_pending_notify`, and the belt path flushes any pending drop on its own next successful send (no double-send) — proven against a ProB-checked Z specification (`docs/notification.tex`, invariant `notifyLost = zfalse`) and an empirical repro (`tests/test_server/test_notify_reliability_repro.py`). The fix's three send sites share one lock (`_notify_lock`) to close a race between a suspenders failure and a concurrent belt send that could otherwise clobber the recorded drop — proven exhaustively against an isolated Z model (`docs/notification-race.tex`, invariants `raceLost = zfalse` and `notifyLost = zfalse`). Each locked send is now bounded by a 3-second timeout (`_NOTIFY_SEND_TIMEOUT`) so a wedged or backpressured MCP transport can no longer hold the lock indefinitely and stall the inbox poller and every tool call behind it — a timeout is handled by the same recorded-drop-and-recover path as a genuine send failure.
...
- **`/biff:read`'s cron cleanup stranded pre-upgrade `/biff:poll` jobs.** A durable auto-poll cron job created before `/biff:poll` was retired into `/biff:read` still fires the now-deleted `/biff:poll` (or dev `/biff-dev:poll-dev`) prompt after upgrade, and the cleanup only matched its own `/biff:read` prompt suffix — so `/biff:read n` (stop) and `/biff:read <duration>` (recreate) both left the stale job running as a duplicate, dead-command loop. Cleanup now also sweeps the legacy `/biff:poll` / `/biff-dev:poll-dev` prompt suffix.
...
- **`/biff:poll` retired.** All functionality folded into `/biff:read`. See Changed above.
...
- **`BIFF_RELAY_URL`, `BIFF_RELAY_TOKEN`, `BIFF_RELAY_NKEYS_SEED`, `BIFF_RELAY_USER_CREDENTIALS`, and `BIFF_RELAY_TLS_HANDSHAKE_FIRST` environment variable overrides for relay configuration, letting a headless invocation (CI, a container, a systemd unit) point `biff` at a private relay with no committed config file** (biff-xvv). Previously `.github/workflows/biff-notify.yml` (and every other CLI command) had no way to reach a token-authed relay — `--relay-url` existed only on `serve`/`mcp` and always cleared auth on override, so CI silently fell back to the bundled demo relay. Env vars sit between `config.local.yaml` and the CLI `--relay-url` override in precedence; a repo that already commits its own `relay.url` in the SHARED `config.yaml` is protected by default and must opt in via `relay.allow_env_override: true` (a personal `config.local.yaml` relay entry never triggers this protection, and a `config.local.yaml` opt-in can never grant it — both are read from the shared file only). Auth env vars are mutually exclusive and replace the file-resolved auth wholesale, never merged; `BIFF_RELAY_NKEYS_SEED`/`BIFF_RELAY_USER_CREDENTIALS` fail fast with a clear error if the named file doesn't exist, instead of surfacing as an opaque NATS connection error. Setting `BIFF_RELAY_URL` always resets the TLS handshake mode and (unless an auth env var also fires) the file-resolved auth to their defaults, so neither can silently carry over onto a different relay; an auth env var with no resolvable relay URL fails fast rather than falling back to the shared demo relay. `.github/workflows/biff-notify.yml`'s notify step now sources `BIFF_RELAY_URL`/`BIFF_RELAY_TLS_HANDSHAKE_FIRST` from repository variables and `BIFF_RELAY_TOKEN` from a repository secret — its `punt-biff==1.15.2` pin predates this feature, so that env block is inert until the pin is bumped in a follow-up release (biff-ykw). Documented in [README.md](README.md#setup) and [docs/INSTALLING.md](docs/INSTALLING.md#environment-variable-relay-overrides-ci-containers-systemd); full design in [`docs/relay-env-overrides.md`](docs/relay-env-overrides.md) and DES-060 in DESIGN.md.

### Changed

- **`RelayAuth`'s `token`/`nkeys_seed`/`user_credentials` fields no longer appear in the dataclass's default `repr()`.** A live secret leak once `BIFF_RELAY_TOKEN` carries CI-sourced secrets through this object — any accidental `logger.debug("%r", config)` or uncaught traceback previously printed the token in clear text.
- **`biff doctor`'s relay-reachability check now shares the same relay resolution chain `wall` and every other CLI command use**, including under `BIFF_RELAY_*` env overrides. Previously `doctor.py` read `config.yaml`/`config.local.yaml` independently, a second reader that PR #383 had to hand-patch by hand once already when `tls_handshake_first` was added.

### Fixed

- **`biff-notify.yml`'s notification message could carry a blank branch name for tag-triggered failures.** `github.event.workflow_run.head_branch` is `null` when the completed workflow was triggered by a tag push (e.g. a `Release` workflow gated on `on: push: tags:`), so the message read `"CI failed: Release on  — <url>"`. `BRANCH` now falls back to `github.event.workflow_run.head_sha` when `head_branch` is null.
- **`_apply_env_relay_overrides` no longer leaks a stale `tls_handshake_first` flag onto a new relay when `BIFF_RELAY_URL` and an auth env var (e.g. `BIFF_RELAY_TOKEN`) are set together.** The reset previously only fired when no auth var accompanied the URL change, but `biff-notify.yml`'s own documented CI shape sets both in the same `env:` block — exactly the case that skipped the reset, reopening the #383 TLS-negotiation footgun. `BIFF_RELAY_TLS_HANDSHAKE_FIRST` also now accepts explicit `"0"`/`"false"`/`"no"` values (previously it could only be turned on, never explicitly forced off, making "unset" and "explicitly false" indistinguishable) so an operator can still re-enable handshake-first for the new relay in the same `env:` block.
- **An auth-only env override with no resolved relay URL now fails fast instead of silently sending the credential to the public demo relay.** If only an auth var (e.g. `BIFF_RELAY_TOKEN`) fired and no relay URL was resolved from `BIFF_RELAY_URL` or the file layers, the token previously fell through to `_apply_demo_relay_default` and was sent to the shared demo relay (`tls://connect.ngs.global`) — a CI misconfiguration as small as a missing or typo'd `vars.BIFF_RELAY_URL` would leak a real secret to a public relay. `_apply_env_relay_overrides` now raises with a clear message instead.
- **The repo-scoping gate on `BIFF_RELAY_*` env vars was keyed on the merged `relay_url` (which includes `config.local.yaml`), blocking any developer with a personal relay entry in their gitignored `config.local.yaml`, not just repos with a team-committed relay.** A new `_ConfigFields.relay_committed` flag tracks whether the SHARED `config.yaml` itself declares `relay.url`, computed before `config.local.yaml` is merged in — the gate (and the `relay.allow_env_override: true` opt-in) now reads this instead of the merged value, matching the documented `config.yaml < config.local.yaml < env vars` precedence.

### Security

- **Hardened the `biff-notify.yml` CI workflow template deployed by `biff enable`.** GitHub Copilot review flagged real issues across the 8 downstream repos this template renders into: `actions/checkout` persisted `GITHUB_TOKEN` credentials into `.git/config` even though the very next step downloads and executes third-party code (`uvx --from punt-biff`) with that token in the job's environment; `uvx --from punt-biff` was unpinned, resolving latest at runtime; `timeout-minutes: 2` was too tight for `setup-uv` + a cold `uvx` install; and `setup-uv` wasn't caching. Fixed all four: added `persist-credentials: false` to the checkout step, pinned `punt-biff==1.15.2` in the `uvx` invocation, raised the timeout to 10 minutes (matching this repo's own workflow convention), and enabled `setup-uv`'s `enable-cache`. Also corrected the `actions/checkout` SHA — the prior pin resolved to `v4.3.1` mislabeled generically as `# v4`; the new pin (`3d3c42e5aac5ba805825da76410c181273ba90b1`) is `v7.0.1`, verified against `actions/checkout`'s tag refs and matching the SHA every other workflow in this repo already uses. Explicitly pinned `ref: ${{ github.event.repository.default_branch }}` on the checkout step rather than the triggering commit's SHA — a security review caught that this job only reads `.punt-labs/biff` (`.punt-labs/biff/enabled`, and `.punt-labs/biff/config.yaml` if the repo has one) to route the notification, so checking out the failing commit (which can be any branch that made a watched workflow fail, not just main) would let that branch introduce or rewrite `.punt-labs/biff/config.yaml` and redirect the notification, and the `github-actions` identity, to an attacker-controlled relay.
- **Reworded `biff-notify.yml`'s checkout-hardening comment to stop overclaiming `.punt-labs/biff/config.yaml` exists in every repo.** A follow-up review on downstream `biff enable` PRs noted most repos deploying this template only have `.punt-labs/biff/enabled` committed, no `config.yaml` — the comment now describes the general risk (an attacker-introduced `.punt-labs/biff/config.yaml`) rather than implying one is always present, and spells out the full paths (matching the workflow's own `sparse-checkout: .punt-labs/biff`) rather than referring to `config.yaml` generically.
- **Every `nats.connect()` call site now redacts auth credentials on connection failure, and no longer chains onto the raw exception at all.** `raise RelayConnectError(...) from None` inside an `except` clause does not clear `__context__` — the original exception, its traceback, and the frame locals it holds stay reachable to anything that walks `__context__` directly (a debugger, an APM integration, `traceback.format_exception(chain=True)`). Worse, `nats.connect(**options)` builds its own, separate `options` dict inside its own frame, so clearing the caller's `auth_kwargs` dict in a `finally` block could never reach it anyway. `NatsRelay._open_connection` (via a new `_dial` method) and `biff doctor`'s connectivity check now record only the failure's type and raise the redacted `RelayConnectError` *after* the `try`/`except`/`finally` handling the raw failure has fully completed — at that point no exception is "currently being handled," so `__context__` is genuinely `None`, not merely display-suppressed. `doctor.py`'s connectivity check no longer constructs a `RelayConnectError` at all — its raw connect failure was always caught and discarded by the retry loop, so there was nothing to redact once no chained exception is left to escape.
- **NATS URLs can carry `user:pass@` userinfo — three places embedded that raw URL verbatim in a warning, an exception message, or a `biff doctor` result instead of the same sanitized `host[:port]` representation `_ConnectionHealth` already used for its own log lines.** Extracted the sanitizer to a module-level `nats_relay.sanitize_relay_url` and wired it into `NatsRelay._dial`'s failure log/exception and `biff doctor`'s `_check_relay` success/failure messages.
...
- **CI coverage for the `ghcr.io/punt-labs/biff-relay` Docker image itself (`tests/test_relay_image/`), closing the gap where the image shipped with zero automated tests and every PR #376 review check was a human running `docker build`/`docker run`/`wget` by hand.** A new `relay-image` job in `.github/workflows/subprocess-tests.yml` builds the image fresh from the PR's own `docker/` tree (never a published tag) on every push/PR, then exercises it end to end against a real running container: presence (`/who`), message write/read (JetStream WORK_QUEUE POP semantics), JetStream persistence across a `docker stop`/`start` cycle, and `entrypoint.sh`'s auth-refusal guard actually exiting 1 (not just Dockerfile parsing) for both a missing `authorization`/`accounts`/`nkeys` block and the unedited placeholder token. Gated behind a new `nats_docker` pytest marker so the default `uv run pytest` does not run it — it requires Docker, the same tradeoff tier 3c makes for a local `nats-server` binary.

### Fixed

- **Documentation across the repo still instructed users to create a `.biff` file, described `biff doctor` as running six checks, and named a `pip install biff`/`uv tool install --editable .` onboarding path.** `.biff` was removed in v1.13.1; the live config is `.punt-labs/biff/config.yaml` + `config.local.yaml`, and `biff doctor` now runs ten checks. Following the README's old "Create a `.biff` file" setup step, or `prfaq.tex`'s onboarding instructions, would not have worked. Corrected README.md, AGENTS.md, TESTING.md, DESIGN.md, DESIGN-INSTALLER.md, CLAUDE.md, docs/INSTALLING.md, docs/TROUBLESHOOTING.md, docs/FAQ.md, docs/CLAUDE_SETUP.md, docs/design-gap-analysis.md, docs/biff_coordination_use_cases_v0.2.md, prfaq.tex, press-release-v0.11.4.tex, and research/agentic-engineering-landscape-2026.md.
- **A nested `claude` process sharing a topmost ancestor PID with its parent session could permanently lock the parent out of every Edit/Write.** A nested `claude` process — spawned explicitly (`claude -p` via a Bash tool call) or automatically (confirmed live: another plugin's own hook spawning a nested `claude` for review) — resolved to the same routing PID as its parent and overwrote the parent's session hint file. The parent's plan-gate then denied every edit for the rest of its life, and `/plan` appeared to succeed while writing to a key the gate would never read; the MCP server's NATS session key was silently wrong too. The session-hint file is now keyed on `CLAUDE_PID` — the owning claude process's own PID, delivered directly by Claude Code since 2.1.234 and distinct per nested process — instead of a process-tree walk that collapsed every nesting level onto one shared PID; falls back to the prior walk only when the env var is absent. (An earlier version of this fix read `CLAUDE_CODE_SESSION_ID` directly instead — that broke across `/clear`, since the long-lived MCP server's own copy of that variable is frozen at spawn time; caught before merge.) See DES-058.
