...
- **Homebrew install option** (`brew tap punt-labs/tap && brew install punt-labs/tap/biff`) documented in [README.md](README.md#quick-start) and [docs/INSTALLING.md](docs/INSTALLING.md#manual-install), alongside the existing `uv tool install`/`pip install` options. The formula itself shipped in `punt-labs/homebrew-tap` PR #33.
