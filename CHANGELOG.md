...
- **`biff enable` and `/biff enable` now produce an identical repo — enable/disable are equivalent on both surfaces (biff-j5u).** The two enable front-ends had diverged: the CLI `biff enable` wrote the committed marker **and** deployed the CI notify workflow **and** deployed the per-clone `.git/hooks/` dispatchers, while the MCP `biff` tool (`/biff enable`) wrote **only** the marker — so a repo enabled via `/biff enable` had the policy marker but no CI workflow and no hooks (or relied on a prior `biff install`). This broke the DES-052 "two equivalent ways to one state" model. Both surfaces now route through a single `RepoEnablement` definition and write/remove exactly the two **committed** artifacts that constitute enablement policy: the marker `.punt-labs/biff/enabled` and the CI workflow `.github/workflows/biff-notify.yml` (it runs on a fresh-checkout CI runner, so it must be tracked). Per-clone git hooks — local, never committed — move to `biff install` (removed by `biff uninstall`); they gate on the marker at runtime, so a fresh clone runs `biff install` once and its hooks stay inert until the committed marker enables the repo. `biff enable` prints a hint pointing at `biff install` when this clone has no hooks. See DES-052.
...
- **`enable` is fail-safe if the CI-workflow write fails (biff-j5u).** `RepoEnablement.enable()` wrote the `.punt-labs/biff/enabled` marker *before* deploying the CI notify workflow, so a failed workflow write (e.g. a read-only `.github/`) left the repo half-enabled: the marker present (biff reads as ON) but no notify workflow. Enable now deploys the CI workflow first and writes the marker last, so any failure leaves the marker absent and the repo cleanly OFF.
