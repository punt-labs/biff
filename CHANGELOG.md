...
- **`set_poll_interval n` silently disabled ALL push detection, contradicting its own description.** The poller task hosting the always-on SUBs was only created when `poll_interval > 0`; it now always runs, with a shared `asyncio.Event` making push-to-refresh latency genuinely interval-independent (including at `interval <= 0`, which waits indefinitely for a wake instead of a periodic timeout). Only the periodic work with no push signal of its own — wall-countdown re-render, stale-invite expiry, the unread backstop — actually degrades when disabled; the tool text now says so honestly, including the real 15x backstop-to-interval ratio.
