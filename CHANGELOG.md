...
- **Every `nats.connect()` call site now redacts auth credentials on connection failure.** A failed connect previously let the raw exception (and, via `__context__`, the plaintext auth kwargs dict in its traceback frame) propagate or sit reachable to any downstream error reporter, debugger, or APM integration. Both `NatsRelay._open_connection` and `biff doctor`'s connectivity check now raise a redacted `RelayConnectError` `from None` and clear the auth kwargs dict in a `finally` block — `from None` alone does not clear `__context__`, so the `finally` clear is what actually removes the raw credentials from the traceback frame.
