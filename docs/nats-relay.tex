...
The recovery transition, and the entire content of the fix.
...
infinite reconnect loop.  $WedgeDetected$ is the \emph{only}
operation enabled in $halfOpen$; removing it (the pre-fix code,
whose \texttt{ping\_interval} default of 240\,s never fired in
practice) makes $halfOpen$ a terminal state.  That terminal state
is biff-tww, and probcli surfaces it as a reachable deadlock.
