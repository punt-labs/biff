...
\texttt{ServerSession} reference (suspenders path).  NATS callbacks
only mutate state (display queue, description-changed flags); the
poller detects the changes and fires the notification from the MCP
server's own coroutine context.
...
callbacks do not fire notifications directly---they are unreliable
from a different coroutine context---but they mutate state and wake
the poller from napping, so the next active tick (2~seconds) detects
the changes and fires the notification.
...
also sets $lastNotifyResult' = notifySent$ ($WallPost$, belt path).
$NatsTalkCallback$ sets $talkDescChanged' = ztrue$ but defers
notification to the poller: it preserves $lastNotifyResult$, and
$PollTick$ detects $talkDescChanged = ztrue$ and sets
...
without producing $notifySent$.  NATS callbacks wake the poller from
napping, ensuring $PollTick$ runs promptly even during idle periods.
...
NATS callbacks do not send notifications directly; they set flags
that the poller consumes.
...
talk from external CLI sets state in the NATS callback, and the
poller fires the notification via the suspenders path (stored session).
Both ultimately produce $lastNotifyResult = notifySent$.
