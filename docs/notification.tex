...
or otherwise cancelled---and removes that inviter's pending entry, whether
it was recorded with its session key (in $talkPending$) or, in the defect
model, without one (in $talkBare$).  Its precondition is the arrival of the
frame on a live connection; it does \emph{not} depend on how long the invite
has been pending.  If no activity survives, the description reverts and the
