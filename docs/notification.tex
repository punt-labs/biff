...
callbacks and KV watchers only mutate state (display queue,
description-changed flags); the poller detects the changes and fires
the notification from the MCP server's own coroutine context.
...
are unreliable from background coroutine contexts---but they mutate
state and wake the poller from napping, so the next active tick
(2~seconds) detects the changes and fires the notification.
...
Fires when the NATS KV watcher detects a wall change from another
user.  Runs \emph{outside} any tool handler context in a background
asyncio task.  Updates the wall description and adds a display queue
item, but does \emph{not} fire an MCP notification directly---sending
notifications from a background task is unreliable for the same reason
as NATS subscription callbacks.

Instead, the watcher wakes the poller from napping so the next active
tick (2~seconds) detects the wall change via its own KV read and fires
the notification from the MCP server's coroutine context.
