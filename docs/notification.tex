...
property once the suspenders failure is properly recorded.
...
established in Section~\ref{sec:sessionloss}, which says that a
suspenders send failure is never silently absorbed.
...
$CaptureSession$ also flushes a $pendingNotify$ recorded before any
session existed---the pre-initialize window in which a description
change has nowhere to send its notification.  This half of the flush
obligation was correct in the original implementation and remains so;
Section~\ref{sec:sessionloss} is about the other half, the flush that
a \emph{mid-session} loss of the reference requires.
...
\begin{schema}{CaptureSession}
  ...
  natsConn' = natsConn \\
  ((pendingNotify = ztrue) \implies \\
  \quad~ (pendingNotify' = zfalse \land
  lastNotifyResult' = notifySent \land \\
  \quad~ \quad~ wallDescChanged' = zfalse \land
  talkDescChanged' = zfalse)) \\
  ((pendingNotify = zfalse) \implies \\
  \quad~ (pendingNotify' = zfalse \land
  lastNotifyResult' = lastNotifyResult \land \\
  \quad~ \quad~ wallDescChanged' = wallDescChanged \land
...
\texttt{tools/list\_changed} notification.  The belt path always
succeeds; the suspenders path does not, and Section~\ref{sec:sessionloss}
models the difference precisely at the operational level ($PollTick$).
This section gives the two paths in their simplified, idealised form
first, exactly as they were understood before the root-cause-\#1
investigation.
...
The real implementation refreshes the stored session reference on
\emph{every} belt call, whether or not the prior reference was still
good---\texttt{\_session = ctx.session}, unconditionally---so
$NotifyBelt$ (re)captures the session regardless of its prior value.
It also flushes any $pendingNotify$ left behind by an earlier
suspenders failure, using the current call's own successful send to
carry the missed notification.  Omitting that flush is the second
half of root cause~\#1 and is kept below as the dead operation
$NotifyBeltStale$.
...
$NotifyBeltStale$ is the defect: an otherwise identical belt call that
recaptures the session and sends successfully---so $lastNotifyResult$
and the changed flags resolve exactly as above---but leaves
$pendingNotify$ untouched.  A drop recorded by an earlier suspenders
failure survives this call unflushed, waiting on some \emph{later}
event to notice it.  Kept here, and proved dead once $notifyLost =
zfalse$ is promoted in Section~\ref{sec:sessionloss}.

\begin{schema}{NotifyBeltStale}
  \Delta NotificationState
  \where
  pendingAge' = pendingAge \\
  toolContext = ztrue \\
...
$PollTickNotifyFail$ is the corrected failure branch: the send fails,
...
connection underneath) re-arrives here.  $RecaptureSession$ models that
second call.  It flushes $pendingNotify$ exactly as $CaptureSession$
does---the shared obligation both capture points owe.
...
Observe the shape of $RecaptureSession$'s second branch: when
$pendingNotify = zfalse$ it changes nothing about the changed flags or
$lastNotifyResult$ at all.  A reconnect that arrives after
$PollTickNotifyFailUnrecorded$ therefore restores $session$ to
$sessCaptured$ but does \emph{not} deliver the notification the client
missed---because nothing recorded that one was owed.  This is root
cause~\#1's second consequence: even the recovery path that exists
specifically to repair a dead session reference cannot repair a debt
...
sessNone \land natsConn = natsConnected$ holds---and confirms the
second half of the finding without needing a further ProB search: by
inspection of $RecaptureSession$'s second implication branch
(triggered because $pendingNotify = zfalse$), firing it sets $session'
= sessCaptured$ while leaving $wallDescChanged'$, $talkDescChanged'$,
and $lastNotifyResult'$ all unchanged.  The reconnect does not recover
the missed notification, because nothing recorded that one was owed.
...
\paragraph{What this does and does not establish.}  Restoring
$notifyLost = zfalse$ and re-running the check exhaustively---the
$DEFAULT\_SETSIZE~2$, $USER~1$ scope, no goal---terminates with ``all
open states visited'': $1{,}086{,}889$ states, $16{,}343{,}296$
transitions, $invariant\_violated{:}0$, $deadlocked{:}0$,
$open{:}0$.  $PollTickNotifyFailUnrecorded$ and $NotifyBeltStale$ are
both reported among the six uncovered operations, alongside the
pre-existing $TalkDrainStale$, $TalkInviteArriveBare$, and
$WithdrawStale$ (Section~\ref{sec:markerfix2}) and
$TalkInviteArriveOverflow$ (live but needs a second $USER$ to trigger,
per Section~\ref{sec:state}'s scoping note)---the same proof shape as
$WithdrawStale$.  This is a \emph{safety} property: it says the
failure-accounting obligation is never skipped, in any reachable
state.  It is deliberately not phrased
as the CTL liveness formula
...
\textsc{true} in \emph{both} the buggy and the fixed model here: $NotifyBelt$
is modelled as unconditionally reliable and unconditionally enabled
whenever $toolContext = ztrue$, so ProB's existential path quantifier
$\mathrm{EF}$ always finds a recovering trace through some future,
unrelated tool call, whether or not the suspenders bookkeeping was
correct.  That is not a false proof, but it is not the property this
bug is about, and stating it as though it were would overclaim what
the check shows---the {\fuzz} discipline is to report exactly what a
green check establishes, not what would be reassuring for it to
establish.  The talk channel's liveness proof works because
$ExpirePendingInvite$ is driven by a clock ($AgeTick$,
$pendingAge$) guaranteed to advance on its own; nothing plays that role
for the suspenders path once $session$ is lost.  The three operations
that restore it---$CaptureSession$, $RecaptureSession$, $NotifyBelt$---are
all environment-triggered (server startup, a client reconnect, a fresh
tool call) rather than time-driven, so no purely background,
poller-only sequence of transitions ever clears a stranded changed
flag.  That structural fact is visible by inspection of the schema set
above: no operation whose guard depends only on $toolContext = zfalse$
sets $session' = sessCaptured$ from $session = sessNone$.  The safety
...
first---discharges it, by inspection of $CaptureSession$'s and
$RecaptureSession$'s first branch.
...
\emph{What remains true}: $WallPost$ and $TalkStart$ (belt path, tool
context) still set $lastNotifyResult' = notifySent$ unconditionally---
the belt path was never the source of this defect.  $KVWallReceive$
and $NatsTalkCallback$ still defer the actual send to the poller, which
is exactly the design that makes $PollTick$'s split
(Section~\ref{sec:sessionloss}) the right place to model the failure.
...
available until $CaptureSession$, $RecaptureSession$, or a fresh
$NotifyBelt$ call restores it.  This is the substance of root
cause~\#1: the property below was the promise the background poller
made to itself, and it is the promise that broke.
...
of $CaptureSession$, $RecaptureSession$, or $NotifyBelt$ arrives next.

\subsection{Wall and Talk Symmetry}

Wall posting uses the belt path (tool context) and always produces
$lastNotifyResult' = notifySent$---the belt path is unconditionally
reliable throughout this specification.  Wall reception from another
user uses $KVWallReceive$, and talk reception uses $NatsTalkCallback$;
both defer the actual send to the poller rather than producing
$notifySent$ themselves.  Section~\ref{sec:sessionloss}'s erratum to
$SuspendersAvailable$ applies equally to both deferred paths: the
poller's next tick attempts the send, and $notifyLost = zfalse$, not
delivery symmetry, is what guarantees the attempt is never silently
lost.
