...
checking rather than a fixed value.  Both carriers are required
non-empty, so each bound is at least one.
...
which supplies two distinct carrier elements throughout while keeping the
queue and unread bounds at two:

\begin{verbatim}
probcli docs/notification.tex -model_check -noltl \
  -p DEFAULT_SETSIZE 2 -p MAX_INITIALISATIONS 16 \
  -p MAX_OPERATIONS 2000
\end{verbatim}

At set size~2 the carriers $QSLOT$ and $USLOT$ each hold two elements, so
$maxQueueSize = maxUnreadCount = 2$; either may be shrunk independently
with \texttt{-card QSLOT}~$n$ or \texttt{-card USLOT}~$n$ without
disturbing the message and key carriers.  $MAX\_INITIALISATIONS$ must
exceed the number of identities $Init$ leaves unconstrained, or ProB
silently drops initial states and reports an incomplete exploration.
...
both at once.  Three suffice, and the proof in
Section~\ref{sec:markerfix2} shows all three are needed.
...
\subsection{Dropping a pending invite}

$DropPendingInvite$ removes one inviter's pending entry---whether it was
recorded with its session key (in $talkPending$) or, in the defect model,
without one (in $talkBare$).  It models two triggers with the same state
effect: an \emph{ntWithdraw} notification from the inviter, and a
time-to-live expiry aged out on a poller tick.  Either way, if no
activity survives, the description reverts.

\begin{schema}{DropPendingInvite}
  \Delta NotificationState \\
  inviter? : USER
  \where
...
$DropPendingInvite$ is the only transition that removes a pending invite
without the recipient's acceptance; without it a marker backed by an
invite from a peer who never returns---exactly the biff-9la
scenario---can never clear on its own, so the talk channel would not be
self-clearing in the way the read reference is.  $TalkAccept$ is the
ordinary progress transition; it is not required for the marker to clear,
but without it the only exit from a pending invite is to drop it, and the
handshake modelled in $talk.tex$ could never complete.
...
superseded: a pending invite must also be removable by
$DropPendingInvite$---an \emph{ntWithdraw} frame from the inviter or a
