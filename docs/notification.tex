...
from a different coroutine context---but they set flags that the
poller picks up within one tick interval (2~seconds).
...
which fires the notification within one tick interval (2~seconds).
...
the poller fires within one tick interval---no changed flag persists
across two consecutive ticks without producing $notifySent$.
