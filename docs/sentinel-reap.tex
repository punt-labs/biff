...
the non-trivial race one.  $resumed$ is a second ghost latch, in the
same style as $sentinelWriteAttempted$: it records only whether
\emph{some} $ResumeOwn$ has fired on this run, and is needed for the
same structural reason $sentinelWriteAttempted$ is --- $exited$ and
$shutdownSignaled$ are themselves reset by $ResumeOwn$
(\S\ref{sec:resume-safe}), so neither can serve as a state predicate
that distinguishes ``reachable only via a resume'' from ``reachable
without one'' once a full shutdown/exit cycle has happened again since
the resume.
...
\texttt{while not shutdown.is\_set()} --- and only while $own$'s process
is still alive ($exited = zfalse$): the tick is a step of $own$'s
\emph{own} \texttt{\_reap\_loop}, which dies with the process, so a
prior $CrashNoSentinel$ (which sets $exited' = ztrue$ in the same step
it sets $crashed' = ztrue$) must disable it too, or a crash-left
sentinel would be reachable for discard by a task that no longer
exists (Cursor Bugbot).
...
\subsection{Resume clears the immediate race, but not the next cycle}
...
or is it correct for a more principled reason?  An earlier draft of
this section answered ``resume never reaches $BadOrphan$,'' using a
goal that constrained the search with $shutdownSignaled = zfalse$ as a
proxy for ``on the resume path.''  That proxy was wrong: $ExitProcess$
requires $shutdownSignaled = ztrue$ as a precondition and never resets
it, and $CrashNoSentinel$ is excluded by the goal's own $crashed =
zfalse$ conjunct, so $exited = ztrue \land shutdownSignaled = zfalse$
is unreachable \emph{by construction}, independent of $ResumeOwn$ ---
deleting $ResumeOwn$ from the model entirely would not change the
result.  The goal was vacuously true and proved nothing about resume
(Cursor Bugbot).

The one thing that \emph{is} true by construction: $rowPresent$ is
touched by exactly two operations on $own$, $ResumeOwn$ (always
\emph{adds} $own$) and $ReleaseOwnSuccess$ (always \emph{removes} it,
together with $own$ from $sentinelPresent$, in the same step), and
$ResumeOwn$'s postcondition sets $exited' = zfalse$ --- so a stale
sentinel discarded by $OwnOrphanReapTick$ in the instant right after a
resume cannot itself satisfy $BadOrphan$'s third conjunct, because
$exited$ is false at that instant.  That much survives the correction.

What does not survive it: resume confers no protection beyond that one
instant.  A resumed incarnation is exactly as exposed to
\S\ref{sec:race}'s five-step race on its \emph{own} subsequent
shutdown as a fresh one is --- resuming only replays $Init$'s effect on
$rowPresent$ and the ghost latches, it does not change how
$OwnOrphanReapTick$, $SignalShutdown$, and $ReleaseOwnFails$ interact.
A ghost latch $resumed$ (added to $SentinelWorld$, set by $ResumeOwn$,
otherwise unchanged by every other operation, in the manner of
$sentinelWriteAttempted$) makes ``reachable via some resume'' a real
state predicate instead of a proxy that collides with the
$exited$/$shutdownSignaled$ machinery:

\begin{verbatim}
probcli docs/sentinel-reap.tex -model_check -noass -nodead \
  -card SESSIONKEY 2 \
  -goal "own : rowPresent & not(own : sentinelPresent) \
         & exited=ztrue & crashed=zfalse & resumed=ztrue"
\end{verbatim}

ProB finds a 15-step witness: $WriteOwnSentinel$, $SignalShutdown$,
$ReleaseOwnFails$, $ExitProcess$ (the first, ordinary exit) --- then
$ResumeOwn$, and from there the same shape as \S\ref{sec:race}'s
five-step race ($WriteOwnSentinel$, $OwnOrphanReapTick$,
$SignalShutdown$, \ldots, $ReleaseOwnFails$, $ExitProcess$) reaches
$BadOrphan$ a second time.  Resume is therefore not an independent
safety mechanism; it is a state reset that a subsequent shutdown can
race exactly as any first shutdown can, and \S\ref{sec:race}'s fix ---
stopping the reaper before the sentinel write, on \emph{every}
incarnation --- is what closes it, not anything specific to resuming.
The two mechanisms the bead named (stale-TTY self-release, unconditional
KV overwrite) are real and both fire in production on resume, but
neither one, nor $ResumeOwn$'s call-order property established above,
extends past that first instant.
...
probcli visits $221$ states ($220$ live plus the search root) via
$1108$ fired transitions, reports \texttt{ALL OPERATIONS COVERED}, zero
