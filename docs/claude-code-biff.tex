...
edit and bash tools. The hook allows only if the plan is set
(for edit/bash tools), or unconditionally for other tools.
...
\begin{schema}{PreToolHookAllow}
  ...
  %
  % WORKFLOW GATE: edit/bash tools require a set plan
  %
...
\begin{enumerate}
  \item \textbf{Plan before editing} (workflow gate): the
    ...
    for edit and bash tools. No path through the state machine can reach
    tool execution for file-modifying tools without a set plan. The gate
    consults the plan alone --- never beads.
...
PreToolUse & Bash & PreToolHookAllow/Deny & Require plan & Yes \\
