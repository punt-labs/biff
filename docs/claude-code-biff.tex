...
\begin{schema}{PreToolHookAllow}
  ...
  %
  % WORKFLOW GATE: edit tools require a set plan (Bash is not gated)
  %
  toolCategory = tcEdit \implies planSet = ztrue \\
  %
...
\begin{enumerate}
  \item \textbf{Plan before editing} (workflow gate): the
    ...
    for edit tools (\texttt{tcEdit}). No path through the state machine
    can reach tool execution for file-modifying tools without a set
    plan. \texttt{Bash} is not gated (the shipped hook matches
    \texttt{Edit|Write} only). The gate consults the plan alone ---
    never beads.
...
PreToolUse & Edit|Write & PreToolHookAllow/Deny & Require plan & Yes \\
...
\begin{enumerate}
  ...
  \item \textbf{Plan is prerequisite for editing}: Once the
    ...
    edit (\texttt{tcEdit}) executes without \texttt{planSet = ztrue}
    (\texttt{Bash} is not gated). This validates the use
