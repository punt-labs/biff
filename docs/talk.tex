...
The two connected-phase foreign-frame operations,
$DrainForeignMessage$ and $DrainForeignEnd$, are the exception to the
set-size-2 scope.  Their guard --- $phase = tpConnected$ with
$(head~queue).nfromKey \neq partnerKey$ --- is a three-party predicate:
it needs $myKey$, the connected $partnerKey$ (which is $\neq myKey$),
and a third \emph{forger} key distinct from both.  At two session keys
the third does not exist, so the operations are enabled in no reachable
state and ProB reports them uncovered; the guarantee they enforce holds
only vacuously there.  Exhausting their behaviour therefore takes a
third session key.  The other carriers are irrelevant to the threat and
may be shrunk to one, which keeps the check small:

\begin{verbatim}
probcli docs/talk.tex -model_check -noltl -coverage \
  -card USER 1 -card TTY 1 -card MSGBODY 1 \
  -card SESSIONKEY 3 -card QSLOT 1 \
  -p MAX_INITIALISATIONS 64 -p MAX_OPERATIONS 4000
\end{verbatim}

At this scope every operation is covered and all $1{,}141$ reachable
states are visited, with no invariant violation and no deadlock: a
forged message or end carrying the third key is drained by
$DrainForeignMessage$ or $DrainForeignEnd$ and never reaches
$DrainMessage$ or $DrainEnd$, so it neither speaks in the partner's
place nor hangs up the call.
...
per-user NATS subject and carry their own $nfrom$, $nfromTty$ and $nfromKey$;
