...
$maxQueueLen$ bounds the notification queue.  It is \emph{not} a fixed
constant of the running system: the implementation's queue is
memory-bounded, and its actual behaviour is documented in DES-043 ---
it is not capped at any small number.  Here $maxQueueLen$ is defined as
the cardinality of an abstract carrier $QSLOT$, the set of available
queue slots, so that the bound is a \emph{scoping parameter} rather
than a hard-wired value.  Because $QSLOT$ is a given set, its size is
chosen by ProB --- uniformly by \texttt{DEFAULT\_SETSIZE}, or
independently of the other carriers by \texttt{-card QSLOT}~$n$.  This
lets an exhaustive check bound the queue small while keeping two
distinct session keys, which is what exercises $MutualAutoAccept$.  The
specification thus asserts only that the queue is bounded by some
natural number; it pins that number to no value the real system does
not enforce.  $maxBodyLen$ is the message body truncation limit.
...
session accepts it (DES-043).  The queue is bounded; overflow drops
the notification.
